...
import { ArrowLeft, Minus, Plus, Trash2, ShoppingCart, CreditCard, Shield, Clock } from 'lucide-react';
import { addons } from '../../data/mockData';

export function Cart({ cartItems, onBack, onUpdateQuantity, onRemoveItem, onCheckout }) {
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              {cartItems.map((item, index) => (
                <Card key={index} className="border border-white/20 bg-white/50 dark:bg-black/50 backdrop-blur-sm">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {item.addons.map((addonName) => {
                                const addon = addons.find(a => a.name === addonName);
                                return (
                                  <Badge key={addonName} variant="secondary" className="text-xs">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="text-lg font-semibold">
                              ${(item.service.price + item.addons.reduce((sum, addonName) => {
                                const addon = addons.find(a => a.name === addonName);
                ...
                <div className="space-y-2">
                  {cartItems.map((item, index) => (
                    <div key={index} className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">{item.service.duration}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
